...
class FirestoreServices{
  ...
  Future<List<String>> fetch_all_topics()async{
    try{
      CollectionReference topicsdata = await FirebaseFirestore.instance.collection('topics');
      List<DocumentSnapshot>  topicinfodocs = (await topicsdata.get()).docs;
      List<String> topics = topicinfodocs.map((e) => e['topic'] as String).toList();
      return topics;
    }catch(e){
      print(e.toString());
      return [];
  ...
  Future<List<MathQuestion>> fetch_math_questions(String topic)async{
    ...
    try{
      Query math_questions = await FirebaseFirestore.instance.collection('JEE MATH').where("tag",isEqualTo: topic);
  ...
  List<MathQuestion> assemble_question_data(List<DocumentSnapshot> math_docs,List<DocumentSnapshot> passage_docs){
    List<MathQuestion> questions=[];
    try{
      questions=math_docs.map((e) => MathQuestion.fromMap(e.data() as Map,passage_docs)).toList();
    }catch(e){
  ...
  Future<List<GraphData>> fetch_math_graphs()async{
    try{
      CollectionReference graphs = await FirebaseFirestore.instance.collection('GRAPHS');
      List<DocumentSnapshot>  graphs_docs = (await graphs.get()).docs;
      List<GraphData> graph_data_list = graphs_docs.map((e) => GraphData.fromMap(e.data() as Map)).toList();
